...
bool CPlyFile5nt::SavePlyFileASCII( std::wstring fileName, bool bOverwrite, 
						bool IncludeNormals, bool IncludeUV,
						bool IncludeRGB, bool IncludeAlpha, 
						bool IncludeTangent, bool IncludeBiNormal )
{
	sSaveAttribs saveAttributes;
	saveAttributes.IncludeNormals = IncludeNormals;
	saveAttributes.IncludeUV = IncludeUV;
	saveAttributes.IncludeRGB = IncludeRGB;
	saveAttributes.IncludeAlpha = IncludeAlpha;
	saveAttributes.IncludeTangent = IncludeTangent;
	saveAttributes.IncludeBiNormal = IncludeBiNormal;

	return this->SavePlyFileASCII( fileName, bOverwrite, saveAttributes );
}

bool CPlyFile5nt::SavePlyFileASCII(std::wstring fileName, bool bOverwrite, const sSaveAttribs &saveAttributes )
{
	...
	// Header
	outFile << L"ply" << std::endl;
	outFile << L"format ascii 1.0" << std::endl;
	outFile << L"comment Created by CPlyFile5nt mfeeney@fanshawec.ca" << std::endl;
	outFile << L"element vertex " << this->GetNumberOfVerticies() << std::endl;

	outFile << L"property float x" << std::endl;
	outFile << L"property float y" << std::endl;
	outFile << L"property float z" << std::endl;

	if ( saveAttributes.IncludeNormals )
	{
		outFile << L"property float nx" << std::endl;
		outFile << L"property float ny" << std::endl;
		outFile << L"property float nz" << std::endl;
	}
	if ( saveAttributes.IncludeRGB )
	{
		outFile << L"property uchar red" << std::endl;
		outFile << L"property uchar green" << std::endl;
		outFile << L"property uchar blue" << std::endl;
	}
	if ( saveAttributes.IncludeAlpha )
	{
		outFile << L"property uchar alpha" << std::endl;
	}
	if ( saveAttributes.IncludeUV )
	{
		outFile << L"property float s" << std::endl;
		outFile << L"property float t" << std::endl;
	}
	if ( saveAttributes.IncludeTangent )
	{
		outFile << L"property float tx" << std::endl;
		outFile << L"property float ty" << std::endl;
		outFile << L"property float tz" << std::endl;
	}
	if ( saveAttributes.IncludeBiNormal )
	{
		outFile << L"property float bx" << std::endl;
		outFile << L"property float by" << std::endl;
		outFile << L"property float bz" << std::endl;
	}


	outFile << L"element face " << this->GetNumberOfElements() << std::endl;
	outFile << L"property list uchar int vertex_indices" << std::endl;
	outFile << L"end_header" << std::endl;


	for ( unsigned int index = 0; index != this->GetNumberOfVerticies(); index++ )
	{
		PlyVertex curVert = this->getVertex_at(index);

		outFile << curVert.xyz.x << " ";
		outFile << curVert.xyz.y << " ";
		outFile << curVert.xyz.z << " ";
		...
		{
			outFile << curVert.red << " ";
			outFile << curVert.green << " ";
			outFile << curVert.blue << " ";
		}

		if ( saveAttributes.IncludeAlpha )
		{
			outFile << curVert.alpha << " ";
		}

		if ( saveAttributes.IncludeUV )
		{
			outFile << curVert.tex0u << " ";
			outFile << curVert.tex0v << " ";
		}

		if ( saveAttributes.IncludeTangent )
		{
			outFile << curVert.tangent.x << " ";
			outFile << curVert.tangent.y << " ";
			outFile << curVert.tangent.z << " ";
		}

		if ( saveAttributes.IncludeBiNormal )
		{
			outFile << curVert.binormal.x << " ";
			outFile << curVert.binormal.y << " ";
			outFile << curVert.binormal.z << " ";
		}
	...
	for ( unsigned int index = 0; index != this->GetNumberOfElements(); index++ )
	{
		outFile << L"3 ";
		outFile << this->getElement_at(index).vertex_index_1 << " ";
		outFile << this->getElement_at(index).vertex_index_2 << " ";
		outFile << this->getElement_at(index).vertex_index_3 << " ";
		outFile << std::endl;
	}
